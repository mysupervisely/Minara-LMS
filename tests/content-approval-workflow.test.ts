import { describe, it, expect, beforeEach } from "vitest";
import { resetDatabase } from "./helpers/reset-db";
import { buildFullScenario, buildAcademicStructure, buildTestUser } from "./helpers/fixtures";
import { createLesson } from "@/services/academic/institution";
import { createAssessment } from "@/services/assessments/assessments";
import {
  submitContentForReview,
  returnContentToDraft,
  approveContent,
  publishContent,
  listContentPendingReviewForProgram,
  listApprovedContent,
  ContentStateError,
} from "@/services/academic/content-workflow";
import { hasRoleForProgram } from "@/services/identity/authorization";
import { getSessionUser } from "@/services/identity/session";
import { login } from "@/services/identity/auth";
import { assignRole } from "@/services/identity/users";
import { listAuditLog } from "@/services/audit/audit";
import { db } from "@/lib/db";

/**
 * Content approval workflow test — Milestone 13's Implementation Plan
 * Phase 2 ("Approval Workflow"), extended by Milestone 14: the same
 * four-state Draft → Submitted → Approved → Published machine, now
 * operating on a specific Lesson/Assessment *Version* rather than the
 * Lesson/Assessment itself, per
 * docs/milestones/milestone-12-curriculum-delivery-vertical-slice/02-content-lifecycle-workflow.md,
 * including the RBAC boundary and the required Return-with-reason step.
 */
describe("Content approval workflow", () => {
  beforeEach(resetDatabase);

  async function draftLessonVersionWithCompetency(
    scenario: Awaited<ReturnType<typeof buildFullScenario>>,
  ) {
    const { lesson, version } = await createLesson(
      {
        courseId: scenario.course.id,
        title: "Workflow Lesson",
        content: "Content.",
        competencyIds: [scenario.competency.id],
      },
      scenario.faculty.id,
    );
    return { lesson, version };
  }

  it("walks a Lesson Version through Draft → Submitted → Approved → Published, and publishing sets the Lesson's publishedVersionId", async () => {
    const scenario = await buildFullScenario();
    const { lesson, version } = await draftLessonVersionWithCompetency(scenario);

    const submitted = await submitContentForReview("LESSON", version.id, scenario.faculty.id);
    expect(submitted.status).toBe("SUBMITTED");

    const approved = await approveContent("LESSON", version.id, scenario.programDirector.id);
    expect(approved.status).toBe("APPROVED");

    const published = await publishContent("LESSON", version.id, scenario.admin.id);
    expect(published.status).toBe("PUBLISHED");
    expect(published.publishedAt).not.toBeNull();

    const updatedLesson = await db.lesson.findUnique({ where: { id: lesson.id } });
    expect(updatedLesson?.publishedVersionId).toBe(version.id);
  });

  it("refuses to submit a Lesson Version with no Competency tagged", async () => {
    const scenario = await buildFullScenario();
    const { version } = await createLesson(
      { courseId: scenario.course.id, title: "Untagged Lesson", content: "Content." },
      scenario.faculty.id,
    );

    await expect(
      submitContentForReview("LESSON", version.id, scenario.faculty.id),
    ).rejects.toThrow(ContentStateError);
  });

  it("submits an Assessment Version for review with no Competency requirement", async () => {
    const scenario = await buildFullScenario();
    const { version } = await createAssessment(
      { courseId: scenario.course.id, title: "Untagged Quiz", instructions: "Instructions." },
      scenario.faculty.id,
    );

    const submitted = await submitContentForReview("ASSESSMENT", version.id, scenario.faculty.id);
    expect(submitted.status).toBe("SUBMITTED");
  });

  it("returns a Submitted version to Draft with a required reason, clearing on resubmission — a returned version remains the version being edited", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);
    await submitContentForReview("LESSON", version.id, scenario.faculty.id);

    const returned = await returnContentToDraft(
      "LESSON",
      version.id,
      "Please clarify the second paragraph.",
      scenario.programDirector.id,
    );
    expect(returned.status).toBe("DRAFT");
    expect(returned.id).toBe(version.id);
    expect(returned.returnReason).toBe("Please clarify the second paragraph.");

    // Resubmitting the SAME version clears the reason — no new version
    // is created by a return/resubmit cycle.
    const resubmitted = await submitContentForReview("LESSON", version.id, scenario.faculty.id);
    expect(resubmitted.id).toBe(version.id);
    expect(resubmitted.returnReason).toBeNull();
  });

  it("rejects an empty reason when returning a version to Draft", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);
    await submitContentForReview("LESSON", version.id, scenario.faculty.id);

    await expect(
      returnContentToDraft("LESSON", version.id, "   ", scenario.programDirector.id),
    ).rejects.toThrow(ContentStateError);
  });

  it("cannot skip states — Draft cannot be approved or published directly", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);

    await expect(approveContent("LESSON", version.id, scenario.programDirector.id)).rejects.toThrow(
      ContentStateError,
    );
    await expect(publishContent("LESSON", version.id, scenario.admin.id)).rejects.toThrow(
      ContentStateError,
    );
  });

  it("cannot approve a version that is not Submitted, and cannot publish a version that is not Approved", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);
    await submitContentForReview("LESSON", version.id, scenario.faculty.id);
    await approveContent("LESSON", version.id, scenario.programDirector.id);

    // Already Approved — approving again should fail.
    await expect(approveContent("LESSON", version.id, scenario.programDirector.id)).rejects.toThrow(
      ContentStateError,
    );

    await publishContent("LESSON", version.id, scenario.admin.id);
    // Already Published — publishing again should fail. This is the
    // guarantee behind "Published Version 1 cannot be modified."
    await expect(publishContent("LESSON", version.id, scenario.admin.id)).rejects.toThrow(
      ContentStateError,
    );
  });

  it("appears in the Program Director's pending-review queue once Submitted, and no longer once Approved", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);
    await submitContentForReview("LESSON", version.id, scenario.faculty.id);

    let pending = await listContentPendingReviewForProgram(scenario.program.id);
    expect(pending.lessonVersions.map((v) => v.id)).toContain(version.id);

    await approveContent("LESSON", version.id, scenario.programDirector.id);
    pending = await listContentPendingReviewForProgram(scenario.program.id);
    expect(pending.lessonVersions.map((v) => v.id)).not.toContain(version.id);

    const approved = await listApprovedContent(scenario.program.id);
    expect(approved.lessonVersions.map((v) => v.id)).toContain(version.id);
  });

  it("a Program Director who does not oversee this Program has no authority to approve it", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);
    await submitContentForReview("LESSON", version.id, scenario.faculty.id);

    const otherStructure = await buildAcademicStructure(scenario.admin.id);
    const otherPD = await buildTestUser({ email: "other-pd-content@example.test" });
    await assignRole({
      userId: otherPD.id,
      role: "PROGRAM_DIRECTOR",
      programId: otherStructure.program.id,
      actorId: scenario.admin.id,
    });

    await login(otherPD.email, "password123");
    const user = (await getSessionUser())!;

    // The authorization boundary the Server Action checks before
    // calling approveContent — see
    // src/app/(portal)/program-director/actions.ts's
    // assertProgramDirectorOwnsContent.
    expect(hasRoleForProgram(user, "PROGRAM_DIRECTOR", scenario.program.id)).toBe(false);
  });

  it("records VERSION_SUBMITTED, VERSION_RETURNED, VERSION_APPROVED, and VERSION_PUBLISHED audit events", async () => {
    const scenario = await buildFullScenario();
    const { version } = await draftLessonVersionWithCompetency(scenario);

    await submitContentForReview("LESSON", version.id, scenario.faculty.id);
    await returnContentToDraft("LESSON", version.id, "Needs work.", scenario.programDirector.id);
    await submitContentForReview("LESSON", version.id, scenario.faculty.id);
    await approveContent("LESSON", version.id, scenario.programDirector.id);
    await publishContent("LESSON", version.id, scenario.admin.id);

    const entries = await listAuditLog();
    const actions = entries.map((e) => e.action);
    expect(actions).toContain("VERSION_SUBMITTED");
    expect(actions).toContain("VERSION_RETURNED");
    expect(actions).toContain("VERSION_APPROVED");
    expect(actions).toContain("VERSION_PUBLISHED");

    const publishEntry = entries.find((e) => e.action === "VERSION_PUBLISHED");
    expect(publishEntry?.actorName).toBe(scenario.admin.name);
  });
});
